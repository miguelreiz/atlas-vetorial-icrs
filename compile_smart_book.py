...
def add_cover_page(doc):
    ...
    run_title = p_title.add_run("Biomecânica Vetorial")
    run_title.font.name = "Calibri"
    ...
    run_editor = p_editor.add_run("Dr. Miguel Reis\nDra. Jordana Sandes")
    ...
    for idx, filepath in enumerate(chapter_files):
        ...
        while line_idx < total_lines:
            ...
            if h_match:
                ...
                # Assign native style correctly to trigger navigation pane outline levels
                p_head.style = doc.styles[f'Heading {level}']
        ...
        # Add specific images dynamically based on the chapter filename
        base_name = os.path.basename(filepath)
        
        images_to_inject = []
        if base_name == "cap01_cornea_biomecanica.md":
            images_to_inject.append((os.path.join(script_dir, "apresentacao", "img", "waxs_fibers.png"), "Estrutura Fibrilar (WAXS)"))
        elif base_name == "cap06_vetor_VR.md":
            images_to_inject.append((os.path.join(script_dir, "apresentacao", "img", "vr_tent.png"), "Vetor Radial (VR) - Efeito Tenda"))
            images_to_inject.append((os.path.join(script_dir, "book_figures", "AVBC_patient_sensitivity.png"), "Sensibilidade Paquimétrica"))
        elif base_name == "cap07_vetor_VT.md":
            images_to_inject.append((os.path.join(script_dir, "apresentacao", "img", "vt_barrel.png"), "Vetor Tangencial (VT) - Efeito Barril"))
            images_to_inject.append((os.path.join(script_dir, "book_figures", "AVBC_arc_sweep.png"), "Efeito do Comprimento de Arco"))
        elif base_name == "cap08_vetor_Vtau.md":
            images_to_inject.append((os.path.join(script_dir, "apresentacao", "img", "vtau_wrench.png"), "Vetor de Torque (Vτ)"))
        elif base_name == "cap10_validacao_fem.md":
            images_to_inject.append((os.path.join(script_dir, "apresentacao", "img", "fem_heatmap.png"), "Simulação por Elementos Finitos (FEM)"))
            
        for img_path, caption in images_to_inject:
...
if __name__ == "__main__":
    ...
    # Path settings
    output_docx = os.path.join(script_dir, "Biomecanica_Vetorial_Mestre.docx")
