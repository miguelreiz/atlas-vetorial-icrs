...
![Figura 10.1: A Analogia dos Faróis — ICE Tipo 1 (Coerente): 3 eixos apontam na mesma direção → imagem nítida, 4.2 linhas de ganho. ICE Tipo 2 (Discordante): eixos divergentes → confusão neural, 1.6 linhas, 35% reintervencão.](../../images/CH-010_ICE/Figura_10.1_ICE_Analogia_Farois.svg)
...
O ICE foi validado retrospectivamente em **300 olhos** com ceratocone submetidos a ICRS, agregando 8 estudos publicados (2013-2024). A validação clínica demonstrou sua superioridade preditiva (AUC 0.82) em relação ao K-max clássico (AUC 0.68) na predição de ganho visual. A metodologia segue o precedente de Santhiago para o PTA: dados publicados recalculados retrospectivamente, sem necessidade de aprovação ética adicional.

#### A Mecânica do ICE e o Paradoxo FEM

🔬 **Validação por Elementos Finitos (FEM):** Simulamos no FEBio 4.12 o mesmo ICRS em uma córnea ectásica. Quando o ICRS é alinhado com o eixo do cone (ICE Alto), o Vetor Radial gera um deslocamento apical de +12.945 µm. Quando o mesmo anel é desalinhado em 90° (ICE Baixo), o aplainamento cai para +10.456 µm. A física por trás do ICE é irrefutável: o Vetor Radial perde 19% de sua eficácia quando atua fora do meridiano de tensão máxima. A análise revela que a dispersão de estresse (DP) no cenário desalinhado é 1.27× maior, o equivalente biomecânico da imprevisibilidade clínica.
...
![Figura 10.2: Curvas ROC Comparativas — ICE-min (AUC 0.82, ouro) supera Kmax (AUC 0.68, azul) e Paquimetria (AUC 0.64, cinza) na predição de ≥3 linhas de ganho BCVA. DeLong p=0.012. N=300, ceratocone + ICRS.](../../images/CH-010_ICE/Figura_10.2_ICE_ROC_AUC.svg)
