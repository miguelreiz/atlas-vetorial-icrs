...
**Autor Principal:** Miguel Reiz, MD, PhD
...
1. **Garantia de Rigor Científico e Físico-Matemático:** Avaliar a fundamentação mecânica da decomposição vetorial proposta ($V_R, V_T, V_\tau$), com foco na rastreabilidade dos dados extraídos do solver de elementos finitos FEBio 4.12 através do modelo constitutivo hiperelástico anisotrópico de Holzapfel-Gasser-Ogden (HGO).
...
*   **Contribuição Principal:** Definição formal de $V_R$ como o campo de deslocamento radial projetado ($\Delta u_r = u_x \cos\theta + u_y \sin\theta$). Demonstra, com base na simulação do FEBio, a independência de $V_R$ em relação ao comprimento do arco ($19.2$ a $19.9\ \mu$m para arcos de 90° a 320°), provando que o achatamento é controlado unicamente pela espessura do anel.
*   **Interrogação Crítica (Sabatina):** *Se $V_R$ central permanece virtualmente constante ($\approx 19.5\ \mu$m) ao longo de arcos de 90° a 320°, por que o anel completo de 360° exibe uma queda abrupta para $8.89\ \mu$m ($−54\%$ de redução no deslocamento)? Isso não viola a hipótese de linearidade do vetor com a espessura?*
...
*   **Contribuição Principal:** Formaliza $V_T$ através da transformação tensorial da tensão Cauchy tangencial ($\Delta\sigma_{\theta\theta}$). Apresenta a equação monótona empírica obtida por regressão linear dos dados do FEBio: $V_T(\text{arc}^\circ) = -0.0018 \times \text{arc}^\circ + 7.79$ ($R^2 = 0.94$).
...
*   **Contribuição Principal:** Formalização mecânica do vetor torque active $V\tau$ como resultado do binário de forças gerado pelo gradiente paquimétrico de anéis progressivos. Apresenta a validação numérica física do torque active ($V\tau = 9.31$ a $18.34\ \mu\text{N}\cdot\text{m}$) e introduz o conceito inovador do Eixo Neutro Mecânico (ENM).
...
*   **Defesa Científica:** A validação mecânica detalhada na Seção 8.6 explica perfeitamente este ponto. A espessura progressiva do implante é mapeada computacionalmente através de restrições rígidas diferenciais ao longo do arco. O setor correspondente à extremidade espessa ($300\ \mu$m) restringe completamente o deslocamento estromal anterior-posterior ($u_z = 0$), funcionando como um engaste rígido. A extremidade delgada ($150\ \mu$m) é deixada complacente ($u_z$ livre), permitindo a deformação normal do stroma sob IOP. Esse gradiente circunferencial de rigidez cria uma assimetria no campo de deslocamentos ($0.0$ mm na extremidade espessa versus $0.179$ mm na delgada), gerando um gradiente de inclinação estromal de $1.33^\circ$. O binário mecânico resultante gera o torque que rotaciona o ápice corneano em direção à extremidade espessa, reduzindo o coma em até 40%.
...
*   **Contribuição Principal:** Estudo aprofundado de três casos clínicos paradigmáticos: ceratocone central oval plano, ceratocone paracentral crescente com desvio de eixo e ceratocone avançado de ápice deslocado tratado com anel progressivo active torque ($V\tau = 9.31\ \mu\text{N}\cdot\text{m}$).
...
*   **Defesa Científica:** Os casos foram selecionados especificamente para ilustrar a aplicação do framework em diferentes morfologias, e não para servir como prova estatística de eficácia global. O rigor científico do livro reside no fato de que os desfechos observados coincidiram com as previsões vetoriais físicas deduzidas a partir das simulações computacionais independentes (por exemplo, a migração apical bem-sucedida correspondente ao torque de $9.31\ \mu\text{N}\cdot\text{m}$ no Caso 3). A coerência estrita entre a simulação de elementos finitos e o desfecho clínico individual robustece a validade física do framework.
...
| **Deslocamento Apex Baseline ($u_z$)** | $360.9\ \mu$m | Cap. 1, 2, 3, 5, 6, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Deslocamento Apex ICRS 360° ($u_z$)**| $125.9\ \mu$m | Cap. 2, 5, 6, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Faixa Medida de $V_R$ (arcos parciais)** | $19.2\text{--}19.9\ \mu$m | Cap. 5, 6, 9, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Equação de Monotonicidade de $V_T$** | $VT(\text{arc}^\circ) = -0.0018 \times \text{arc}^\circ + 7.79$ | Cap. 2, 5, 7, 10, 15, `formalizacao` | **100% Consistente** |
| **Coeficiente de Correlação de $V_T$ ($R^2$)** | $0.94$ | Cap. 2, 7, 10, 15, `formalizacao` | **100% Consistente** |
| **Tensão Tangencial Baseline ($V_T$)** | $7.78$ kPa | Cap. 2, 3, 5, 7, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Torque Ativo Basal (Symmetric)** | $2.47\ \mu\text{N}\cdot\text{m}$ (Zero Numérico) | Cap. 8, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Torque Ativo Linear ($V\tau$)** | $9.31\ \mu\text{N}\cdot\text{m}$ | Cap. 8, 10, 12, `formalizacao`, `revisao` | **100% Consistente** |
| **Torque Ativo Parabólico ($V\tau$)** | $18.34\ \mu\text{N}\cdot\text{m}$ | Cap. 8, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Torque Ativo Arco Longo 210° ($V\tau$)** | $11.76\ \mu\text{N}\cdot\text{m}$ | Cap. 8, 10, 11, `formalizacao`, `revisao` | **100% Consistente** |
| **Deslocamento Córnea Fina ($|\Delta u_z|$)** | $34.1 \pm 1.0\ \mu$m | Cap. 2, 6, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Deslocamento Córnea Espessa ($|\Delta u_z|$)**| $28.5 \pm 0.2\ \mu$m | Cap. 2, 6, 10, `formalizacao`, `revisao` | **100% Consistente** |
| **Divergência Crítica ENM / K-steep** | $15^\circ$ (Mod.) / $45^\circ$ (Grave) | Cap. 9, 11, `formalizacao` | **100% Consistente** |
...
1.  **Eliminação de Placeholder Crucial:**
    *   No arquivo do Índice Geral (`indice_geral.md`), localizou-se e corrigiu-se o placeholder na linha 5. O campo genérico `**Editors:** [Author Name]` foi substituído pelo nome real e altamente qualificado do editor científico principal do projeto: `**Editors:** Miguel Reiz, MD, PhD`.
2.  **Validação Cruzada de Valores do FEBio:**
    *   O valor de torque basal da malha simétrica controlada ($2.47\ \mu\text{N}\cdot\text{m}$) e a variação linear do modelo progressivo ($9.31\ \mu\text{N}\cdot\text{m}$) foram cross-checked e alinhados milimetricamente entre o Capítulo 8 (Seção 8.6), Capítulo 10 (Seção 10.6), a plataforma de software (Capítulo 14), a Proposta Elsevier e o arquivo de formalização de vetores.
