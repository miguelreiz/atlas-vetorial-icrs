...
![Figura 6.1 — Vetor VT: mecanismo de cintagem circunferencial (hoop restriction).](book_figures/fig_06_01_cintagem.svg)
...
### 6.2.2 Extração do Vetor VT (δ_ring)

Para quantificar matematicamente este efeito de restrição, o modelo extrai o parâmetro δ_ring — o deslocamento radial médio ao longo do anel de 360° da zona de implante (raio = 2.5 mm). 
1. Num olho normal sem anel, a deformação δ_ring sob PIO é máxima (linha de base).
2. Onde o ICRS de PMMA reside, o δ_ring local é 0 (incompressível no eixo XY).
3. À medida que o arco aumenta, a percentagem de δ_ring = 0 aumenta, reduzindo a média circunferencial linearmente.

Quanto menor for o δ_ring global, maior é a força de cintagem restritiva a forçar a córnea de volta para um formato regular. O V_T é diretamente inversamente proporcional a este deslocamento: **Mais contenção = Maior efeito Vetor Tangencial.**
...
**Tabela 7.1.** Deslocamento circunferencial médio ao longo da zona de implante (δ_ring) em função do arco.
| Configuração | Arco (°) | δ_ring (μm) | % Restrição face à base |
|--------------|---------|-------------|-------------------------|
...
A equação fundamental do V_T (onde V_T reflete a contenção Δδ_ring) segue a regressão linear:
**Restrição** ∝ **Arco** (R² > 0.99).

![Figura 6.2 — Vetor VT: regressão linear δ_ring vs comprimento de arco (R² > 0,99).](book_figures/fig_06_02_regressao_vt.svg)
...
Enquanto a injeção de espessura impulsiona massivamente o V_R (Aplanamento miópico vertical, Capítulo 5), a contenção XY é quase alheia ao volume inserido no eixo Z. A contenção existe pela simples presença física do acrílico que impede a lona corneana de deslizar horizontalmente. 
Isto significa que um segmento de 320° com 150 μm (fino) regularizará a superfície quase exatamente da mesma forma que um de 320° com 300 μm (espesso). No entanto, o de 300 μm infligirá um V_R dramático.
...
1. **Passo 1: Definir a Regularização (V_T).**
 ...
 - *Se elevado (> 4.0 D)*: Arcos muito longos (210° a 320°) aplicam uma restrição férrea massiva (δ_ring < 10 μm), esmagando a assimetria comática.
 
2. **Passo 2: Definir o Aplanamento (V_R).**
 - Qual a magnitude do Erro Esférico / K-médio miópico alvo?
 - *Se for precisa alta refração*: Seleciona-se espessuras monstruosas (300 μm, 350 μm), independentemente do arco escolhido no Passo 1.
 - *Se o K-max for baixo*: Selecionam-se espessuras finas (150 μm), para que o longo arco esculpa a córnea sem induzir hipermetropia.
...
3. A nossa análise demonstra uma correlação matemática quase perfeita, inversamente linear, entre o Comprimento de Arco e a capacidade da córnea se deformar (R² > 0.99).
